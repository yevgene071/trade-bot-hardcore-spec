...
class BreakoutEatThrough : public IStrategy {
  public:
    struct Config {
        ...
        double leader_neutral_max_pct{0.10}; // |leader_change_5s| <= this is neutral, not contra
        double min_tape_aggression{0.3};     // Tape directionality (buy-sell)/total
        double min_relative_volume{1.5};     // Volume surge vs 30s avg
        double max_resistance_cluster_ratio{
            0.7}; // Max ratio of resistance clusters vs our entry density
        double leader_pre_entry_contra_pct{
            0.10}; // |lag_pct| percent threshold for pre-entry LeaderMove contra
        ...
        std::chrono::seconds fade_contra_exit_sec{
            5}; // TapeFade on our side within this window → close
        std::chrono::seconds leader_contra_exit_sec{
            5};                              // LeaderMove contra within this window → close
  ...
  private:
    ...
    FixedString<64> last_reject_reason_;
    std::chrono::system_clock::time_point last_reject_time_{};
