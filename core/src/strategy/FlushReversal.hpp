...
class FlushReversal : public IStrategy {
  public:
    struct Config {
        // FN-004: the current implementation is a gated paper/offline prototype.
        // Live-grade execution is phase-later until T5 wires LiquidationFlush,
        // open-interest/history evidence, and corresponding automated tests.
        bool allow_live{false};
        bool live_gates_implemented{false};
        bool require_liquidation_flush{true};
        bool require_open_interest_confirmation{true};
    ...
    static bool live_gate_satisfied(const TradePlan& plan, const Config& cfg) noexcept {
        (void)plan;
        (void)cfg;
        // FN-004: no current plan evidence can prove LiquidationFlush + OI/history.
        // Keep live FlushReversal disabled until T5 implements and tests those fields.
        return false;
    }

    // Set TickerUniverse for tier-based thresholds
    void set_universe(const TickerUniverse* universe) {
        universe_ = universe;
    }
    ...
    const Ticker& ticker() const override {
        return ticker_;
    }

    void on_frame(const FeatureFrame& frame) override;
    std::optional<TradePlan> on_signal(const Signal& signal,
    ...
    std::optional<FixedString<32>>
    check_close_conditions(const FeatureFrame& latest_frame) override;
