...
void PaperExecutor::tick(std::chrono::system_clock::time_point now) {
    // B3-FIX: Hold lock while snapshotting the active ticker list
    // to avoid data race with on_trade/on_orderbook_* callbacks.
    ...
    if (pit != positions_.end()) {
        ...
        if (bit != books_.end()) {
            ...
            if (bid > 0.0 && ask > 0.0) {
                ...
                // Post-entry invalidation — mirrors LiveExecutor (§ 0.4, § 2.7)
                if (!exit) {
                    const double mid = (bid + ask) * 0.5;
                    const double elapsed_sec = std::chrono::duration<double>(now - pos.opened_at).count();

                    // § 0.4 no_progress_timeout: close if price hasn't moved in our favour
                    if (elapsed_sec > pos.plan.no_progress_timeout_sec) {
                        const bool no_progress =
                            (pos.plan.side == Side::Buy  && mid <= pos.avg_price * 1.0001) ||
                            (pos.plan.side == Side::Sell && mid >= pos.avg_price * 0.9999);
                        if (no_progress) {
                            LOG_WARN("PaperExecutor: no_progress_timeout ({:.0f}s) for {} {} — closing",
                                     pos.plan.no_progress_timeout_sec, ticker,
                                     pos.plan.side == Side::Buy ? "LONG" : "SHORT");
                            exit = true;
                            exit_price = pos.plan.side == Side::Buy ? bid : ask;
                            reason = "NoProgress";
                        }
                    ...
                    // § 2.7 follow-through: close if move insufficient after grace period
                    if (!exit && elapsed_sec > pos.plan.post_entry_grace_sec &&
                        pos.plan.min_follow_through_bps > 0.0) {
                        const double move_bps =
                            (pos.plan.side == Side::Buy)
                            ? (mid - pos.avg_price) / pos.avg_price * 10000.0
                            : (pos.avg_price - mid) / pos.avg_price * 10000.0;
                        if (move_bps < pos.plan.min_follow_through_bps) {
                            LOG_WARN("PaperExecutor: follow-through {:.1f} bps < min {:.1f} bps for {} {} — closing",
                                     move_bps, pos.plan.min_follow_through_bps, ticker,
                                     pos.plan.side == Side::Buy ? "LONG" : "SHORT");
                            exit = true;
                            exit_price = pos.plan.side == Side::Buy ? bid : ask;
                            reason = "FollowThrough";
                        }
                    }
                }
...
void PaperExecutor::inject_recovered_trades(const std::vector<ActiveTrade>& trades) {
    ...
    for (const auto& t : trades) {
        if (t.state == TradeState::Open || t.state == TradeState::Exiting) {
            positions_[t.plan.ticker] = Position{t.plan, t.executed_size, t.avg_entry_price, t.opened_at};
