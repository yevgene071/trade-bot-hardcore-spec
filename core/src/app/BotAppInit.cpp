...
BotApp::BotApp(boost::asio::io_context& ioc)
    : ioc_(ioc), timer_(ioc), dashboard_timer_(ioc), pool_fallback_timer_(ioc),
      signal_level_cleanup_timer_(ioc), last_reset_day_(TradingDay::current_date_utc()),
      ...
      kill_switch_(&KillSwitch::instance()), system_clock_(std::make_shared<WallClock>()) {}
...
BotApp::~BotApp() {
    ...
    // Clear registry to destroy external clients and their Asio timers safely while
    // ExternalIoContext is still running.
...
void BotApp::init_config_and_logger_() {
    ...
    Logger::init(Config::get_or<std::string>("logger.path", "logs/trade_bot.log"),
...
void BotApp::register_strategy_affinities_(std::shared_ptr<double> exchange_mult) {
    ...
    double bounce_min_vol =
        Config::get_or<double>("universe.affinity.bounce.min_volume_24h_usd", 100000000.0);
    double bounce_max_spread =
        Config::get_or<double>("universe.affinity.bounce.max_avg_spread_bps", 3.0);
    double bounce_max_funding =
        Config::get_or<double>("universe.affinity.bounce.max_funding_rate_bps", 2.0);
    double bounce_max_vol_1min =
        Config::get_or<double>("universe.affinity.bounce.max_volatility_1min_bps", 30.0);
    universe_.register_strategy("bounce", [this, bounce_min_vol, bounce_max_spread,
                                           bounce_max_funding, bounce_max_vol_1min,
                                           exchange_mult](const Ticker& t) {
        auto stats = universe_.get_stats(t);
        if (!stats)
            return true;
        if (stats->volume_24h_usd > 0.0 &&
            stats->volume_24h_usd < bounce_min_vol * (*exchange_mult))
            return false;
        if (stats->avg_spread_bps > 0.0 && stats->avg_spread_bps > bounce_max_spread)
            return false;
        if (std::abs(stats->funding_rate_bps) > bounce_max_funding)
            return false;
        if (stats->volatility_1min_bps > 0.0 && stats->volatility_1min_bps > bounce_max_vol_1min)
            return false;
        return true;
    });

    double breakout_min_vol =
        Config::get_or<double>("universe.affinity.breakout.min_volume_24h_usd", 100000000.0);
    double breakout_max_spread =
        Config::get_or<double>("universe.affinity.breakout.max_avg_spread_bps", 4.0);
    double breakout_max_funding =
        Config::get_or<double>("universe.affinity.breakout.max_funding_rate_bps", 5.0);
    universe_.register_strategy("breakout", [this, breakout_min_vol, breakout_max_spread,
                                             breakout_max_funding, exchange_mult](const Ticker& t) {
        auto stats = universe_.get_stats(t);
        if (!stats)
            return true;
        if (stats->volume_24h_usd > 0.0 &&
            stats->volume_24h_usd < breakout_min_vol * (*exchange_mult))
            return false;
        if (stats->avg_spread_bps > 0.0 && stats->avg_spread_bps > breakout_max_spread)
            return false;
        if (std::abs(stats->funding_rate_bps) > breakout_max_funding)
            return false;
        return true;
    });

    double leaderlag_min_vol =
        Config::get_or<double>("universe.affinity.leaderlag.min_volume_24h_usd", 50000000.0);
    double leaderlag_max_spread =
        Config::get_or<double>("universe.affinity.leaderlag.max_avg_spread_bps", 3.0);
    double leaderlag_max_funding =
        Config::get_or<double>("universe.affinity.leaderlag.max_funding_rate_bps", 3.0);
    double leaderlag_min_vol_1min =
        Config::get_or<double>("universe.affinity.leaderlag.min_volatility_1min_bps", 3.0);
    universe_.register_strategy("leaderlag", [this, leaderlag_min_vol, leaderlag_max_spread,
                                              leaderlag_max_funding, leaderlag_min_vol_1min,
                                              exchange_mult](const Ticker& t) {
        auto stats = universe_.get_stats(t);
        if (!stats)
            return true;
        if (stats->volume_24h_usd > 0.0 &&
            stats->volume_24h_usd < leaderlag_min_vol * (*exchange_mult))
            return false;
        if (stats->avg_spread_bps > 0.0 && stats->avg_spread_bps > leaderlag_max_spread)
            return false;
        if (std::abs(stats->funding_rate_bps) > leaderlag_max_funding)
            return false;
        // LeaderLag needs momentum: too little volatility = no lag signal
        if (stats->volatility_1min_bps > 0.0 && stats->volatility_1min_bps < leaderlag_min_vol_1min)
            return false;
        return true;
    });
    ...
    universe_.register_strategy("flushreversal", [this, flush_min_vol, flush_max_spread,
        ...
        if (!stats)
            return true;
        if (stats->volume_24h_usd > 0.0 && stats->volume_24h_usd < flush_min_vol * (*exchange_mult))
            return false;
        if (stats->avg_spread_bps > 0.0 && stats->avg_spread_bps > flush_max_spread)
            return false;
        // Extreme funding = very crowded trade, flush reversals in crowded markets fail often
        if (std::abs(stats->funding_rate_bps) > flush_max_funding)
            return false;
        return true;
    });
}
        ...
        if (token == "order") {
            ...
            if (iss >> action >> ticker) {
                ...
                auto to_short = [](const std::string& cls) -> std::string {
                    if (cls == "BounceFromDensity")
                        return "bounce";
                    if (cls == "BreakoutEatThrough")
                        return "breakout";
                    if (cls == "LeaderLag")
                        return "leaderlag";
                    if (cls == "FlushReversal")
                        return "flushreversal";
...
void BotApp::subscribe_signal_bus_() {
    ...
    signal_bus_->subscribe([&, dash_cooloffs = std::move(dash_cooloffs)](const Signal& s) mutable {
        MetricsRegistry::instance().counter_inc(
            "trade_bot_signals_total", {{"kind", std::to_string(static_cast<int>(s.kind))}});
        ...
        if (dash_cooloffs.size() > 500) {
            ...
            for (auto map_it = dash_cooloffs.begin(); map_it != dash_cooloffs.end();) {
                if (map_it->second < cutoff)
                    map_it = dash_cooloffs.erase(map_it);
                else
                    ++map_it;
            }
        }
        ...
        gmtime_r(&tt, &tm_buf);
        char tbuf[16];
        std::snprintf(tbuf, sizeof(tbuf), "%02d:%02d:%02d.%03d", tm_buf.tm_hour, tm_buf.tm_min,
...
RiskManager::Config BotApp::load_risk_config_() {
    ...
    rm.max_concurrent_positions =
        static_cast<int>(Config::get_or<int64_t>("risk.max_concurrent_positions", 3));
    ...
    rm.news_calendar_check_min =
        static_cast<int>(Config::get_or<int64_t>("risk.news_calendar_check_min", 60));
    rm.news_calendar_require_fresh =
        Config::get_or<bool>("risk.news_calendar_require_fresh", false);
    rm.funding_blackout_pre_sec =
        static_cast<int>(Config::get_or<int64_t>("risk.funding_blackout_pre_sec", 30));
    rm.funding_blackout_post_sec =
        static_cast<int>(Config::get_or<int64_t>("risk.funding_blackout_post_sec", 30));
    ...
    rm.max_positions_per_ticker =
        static_cast<int>(Config::get_or<int64_t>("risk.max_positions_per_ticker", 1));
    return rm;
}
...
BotApp::StrategyConfigs BotApp::load_strategy_configs_() {
    ...
    c.bounce.burst_contra_exit_sec =
        std::chrono::seconds(Config::get_or<int64_t>("strategies.bounce.burst_contra_exit_sec", 5));
    c.bounce.min_density_age_ms = std::chrono::milliseconds(
        Config::get_or<int64_t>("strategies.bounce.min_density_age_ms", 5000));
    c.bounce.no_progress_timeout_sec =
        Config::get_or<double>("strategies.bounce.no_progress_timeout_sec", 120.0);

    c.breakout.max_avg_spread_bps =
        Config::get_or<double>("strategies.breakout.max_avg_spread_bps", 4.0);
    c.breakout.aggressive_offset_bps =
        Config::get_or<double>("strategies.breakout.aggressive_offset_bps", 2.0);
    c.breakout.stop_buffer_bps = Config::get_or<double>("strategies.breakout.stop_buffer_bps", 5.0);
    c.breakout.tp1_r = Config::get_or<double>("strategies.breakout.tp1_r", 1.5);
    c.breakout.tp1_size_ratio = Config::get_or<double>("strategies.breakout.tp1_size_ratio", 0.5);
    c.breakout.min_leader_correlation =
        Config::get_or<double>("strategies.breakout.min_leader_correlation", 0.6);
    c.breakout.leader_neutral_max_pct =
        Config::get_or<double>("strategies.breakout.leader_neutral_max_pct", 0.10);
    c.breakout.min_tape_aggression =
        Config::get_or<double>("strategies.breakout.min_tape_aggression", 0.3);
    c.breakout.min_relative_volume =
        Config::get_or<double>("strategies.breakout.min_relative_volume", 1.5);
    c.breakout.max_resistance_cluster_ratio =
        Config::get_or<double>("strategies.breakout.max_resistance_cluster_ratio", 0.7);
    c.breakout.leader_pre_entry_contra_pct =
        Config::get_or<double>("strategies.breakout.leader_pre_entry_contra_pct", 0.10);
    c.breakout.support_search_range_bps =
        Config::get_or<double>("strategies.breakout.support_search_range_bps", 20.0);
    c.breakout.min_distance_from_best_bps =
        Config::get_or<double>("strategies.breakout.min_distance_from_best_bps", 2.0);
    c.breakout.tier_min_tape_aggression = Config::get_or<std::vector<double>>(
        "strategies.breakout.tiers.min_tape_aggression", c.breakout.tier_min_tape_aggression);
    c.breakout.tier_min_distance_from_best_bps =
        Config::get_or<std::vector<double>>("strategies.breakout.tiers.min_distance_from_best_bps",
                                            c.breakout.tier_min_distance_from_best_bps);
    c.breakout.tier_support_search_range_bps =
        Config::get_or<std::vector<double>>("strategies.breakout.tiers.support_search_range_bps",
                                            c.breakout.tier_support_search_range_bps);
    c.breakout.fade_contra_exit_sec = std::chrono::seconds(
        Config::get_or<int64_t>("strategies.breakout.fade_contra_exit_sec", 5));
    c.breakout.leader_contra_exit_sec = std::chrono::seconds(
        Config::get_or<int64_t>("strategies.breakout.leader_contra_exit_sec", 5));
    c.breakout.leader_exit_contra_pct =
        Config::get_or<double>("strategies.breakout.leader_exit_contra_pct", 0.15);
    c.breakout.no_progress_timeout_sec =
        Config::get_or<double>("strategies.breakout.no_progress_timeout_sec", 60.0);
    c.breakout.post_entry_grace_sec =
        Config::get_or<double>("strategies.breakout.post_entry_grace_sec", 5.0);
    ...
    c.leaderlag.correlation_exit_threshold =
        Config::get_or<double>("strategies.leaderlag.correlation_exit_threshold", 0.3);
    c.leaderlag.leader_exit_reversal_bps =
        Config::get_or<double>("strategies.leaderlag.leader_exit_reversal_bps", 5.0);
    c.leaderlag.swing_lookback_ticks =
        static_cast<int>(Config::get_or<int64_t>("strategies.leaderlag.swing_lookback_ticks", 30));
    c.leaderlag.no_progress_timeout_sec =
        Config::get_or<double>("strategies.leaderlag.no_progress_timeout_sec", 15.0);
    ...
    c.flushreversal.min_flush_count = static_cast<int>(get_flush_i64("min_flush_count", 2));
    c.flushreversal.flush_count_window =
        std::chrono::seconds(get_flush_i64("flush_window_sec", 120));
    c.flushreversal.entry_timeout = std::chrono::seconds(get_flush_i64("entry_timeout_sec", 8));
    c.flushreversal.min_level_touches = static_cast<int>(get_flush_i64("min_level_touches", 2));
    return c;
}
...
void BotApp::init_feed_and_executor_(std::shared_ptr<BeastWsClient> ws, int port, int connection_id,
    ...
    feed_->set_orderbook_snapshot_fetcher(
        [gateway, connection_id](const Ticker& ticker, int zoom_index,
                                 std::optional<int> depth_levels,
                                 std::optional<double> depth_percent) {
            return gateway->get_orderbook_snapshot(connection_id, ticker, zoom_index, depth_levels,
                                                   depth_percent);
        });
    feed_->add_listener(finres_handler_.get());
    m_connection_id_for_tick = connection_id;
    feed_->set_record_tap(
        [this](const nlohmann::json& msg, int64_t ts_ns) { dump_recorder_.record(msg, ts_ns); });
    ...
    for (const auto& entry : recovery_res.log_entries)
        LOG_INFO("[Recovery] {}", entry);
...
void BotApp::setup_strategy_engine_callbacks_() {
    strategy_engine_->set_on_plan([&](const TradePlan& plan) {
        ...
        static auto& risk_to_submit_hist =
            PerfRegistry::instance().get_or_create(kStageRiskToSubmit);
        ...
        if (decision.accepted) {
            ...
            LOG_INFO("[Strategy] SUBMITTING plan for {}: {} @ {}, size={}", plan.ticker,
                     plan.side == Side::Buy ? "BUY" : "SELL", accepted_plan.entry_price,
                     accepted_plan.size_coin);
            {
            ...
            LOG_INFO("[Risk] Plan REJECTED for {}: {} ({})", plan.ticker,
                     static_cast<int>(decision.reason), decision.details);
            strategy_engine_->reset_strategy_plan(plan.ticker, plan.strategy_name.c_str());
        }
    });
...
void BotApp::setup_affinity_handler_(BounceFromDensity::Config bounce_cfg,
    ...
    universe_.set_affinity_change_handler([&, bounce_cfg, breakout_cfg, leaderlag_cfg,
                                           flushreversal_cfg](const Ticker& ticker,
                                                              const std::string& strategy,
                                                              bool enabled) {
        LOG_INFO("[Universe] {} {} for {}", enabled ? "ENABLED" : "DISABLED", strategy, ticker);
        if (enabled) {
            if (!controllers_.contains(ticker)) {
                std::string leader_name = normalize_ticker_(Config::get_or<std::string>(
                    "universe.affinity.leaderlag.require_leader", "BTC_USDT"));
                ...
                if (processor_) {
                    ...
                    processor_->register_ticker(ticker, ctrl->book.get(), ctrl->stream.get(),
                                                ctrl->leader_tracker.get(), ctrl->extractor.get(),
                                                ctrl.get());
                ...
                if (ob_settings_loader_) {
                    try {
                        ...
                        LOG_WARN("[Universe] ob_settings fetch failed for {}: {}", ticker,
                                 e.what());
                    }
                }
                if (leader_name != ticker && !controllers_.contains(leader_name)) {
                    ...
                    if (processor_) {
                        ...
                        processor_->register_ticker(leader_name, ctrl->book.get(),
                                                    ctrl->stream.get(), ctrl->leader_tracker.get(),
                                                    ctrl->extractor.get(), ctrl.get());
                        LOG_INFO("[PipelineProcessor] Registered components for dynamic leader {}",
                                 leader_name);
                    }
                }
            }
            ...
            TickerInfo info{
                ticker,        "",           "", true, meta.price_increment, meta.size_increment,
                meta.min_size, meta.max_size};

            // Per-ticker scaling: sqrt(sf) smooths extremes (BTC sf≈2.2→×1.48, small alt
            // sf≈0.05→×0.22)
            ...
            if (strategy == "bounce") {
                ...
                cfg.no_progress_timeout_sec =
                    std::min(cfg.no_progress_timeout_sec * inv_sqrt, 300.0);
                ...
                cfg.no_progress_timeout_sec =
                    std::min(cfg.no_progress_timeout_sec * inv_sqrt, 120.0);
                ...
                cfg.no_progress_timeout_sec =
                    std::min(cfg.no_progress_timeout_sec * inv_sqrt, 60.0);
            ...
            std::string cls;
            if (strategy == "bounce")
                cls = "BounceFromDensity";
            else if (strategy == "breakout")
                cls = "BreakoutEatThrough";
            else if (strategy == "leaderlag")
                cls = "LeaderLag";
            else if (strategy == "flushreversal")
                cls = "FlushReversal";
            if (!cls.empty())
                strategy_engine_->remove_strategy(ticker, cls);
...
std::vector<Ticker> BotApp::init_universe_pool_(std::shared_ptr<OrderGateway> gateway,
    ...
    try {
        ...
        for (auto& info : all_infos) {
            if (!info.is_trading_allowed)
                continue;
            if (!static_flt.accepts(info.name))
                continue;
            universe_.cache_meta(info.name, TickerMeta{info.price_increment, info.size_increment,
                                                       info.min_size, info.max_size});
            all_candidates.push_back(info.name);
        }
        LOG_INFO("[Universe] {}/{} tickers pass static filter", all_candidates.size(),
                 all_infos.size());
    } catch (const std::exception& e) {
    ...
    u_cfg.max_pool_size =
        static_cast<std::size_t>(Config::get_or<int64_t>("universe.pool.max_pool_size", 30));
    ...
    u_cfg.dynamic_thresholds_reference_volume = Config::get_or<double>(
        "universe.dynamic_thresholds.reference_volume_24h_usd", 500'000'000.0);
...
void BotApp::start_notification_feed_(int port) {
    ...
    notif_cfg.exchange_id =
        static_cast<int>(Config::get_or<int64_t>("notifications.exchange_id", 0));
    notif_cfg.market_type =
        static_cast<int>(Config::get_or<int64_t>("notifications.market_type", 0));
    ...
    LOG_INFO("[Universe] NotificationFeed started (notifications={}, signal_levels={})",
             notif_cfg.subscribe,
             notif_cfg.signal_levels_subscribe && signal_level_bridge_ != nullptr);
}
...
void BotApp::schedule_signal_level_cleanup_() {
    ...
    const auto interval_min =
        Config::get_or<int64_t>("signals.level_bridge.cleanup_interval_min", 60);
    if (interval_min <= 0)
        return;
    ...
    signal_level_cleanup_timer_.async_wait([this](const boost::system::error_code& ec) {
        if (ec || shutdown_initiated_.load(std::memory_order_relaxed))
            return;
        if (signal_level_gateway_) {
            try {
...
void BotApp::init_components() {
    ...
    clock_monitor_ = std::make_unique<ClockDriftMonitor>(
        ntp, ClockDriftMonitor::Config{
                 .sources = Config::get_or<std::vector<std::string>>(
                     "clock.sources", {"pool.ntp.org", "time.google.com"}),
                 .check_interval =
                     std::chrono::seconds(Config::get_or<int64_t>("clock.check_interval_sec", 30)),
                 .query_timeout = std::chrono::milliseconds(
                     Config::get_or<int64_t>("clock.query_timeout_ms", 1500)),
    ...
    risk_manager_ =
        std::make_unique<RiskManager>(universe_, news_, load_risk_config_(), system_clock_);
    load_account_state_();
    maybe_load_news_();
    ...
    if (!port)
        throw std::runtime_error("MetaScalp not found");
    http_client_->set_timeout_ms(15000);
    ...
    *exchange_mult = conn_info.exchange_volume_mult;
    ...
    if (Config::get_or<bool>("signals.level_bridge.enabled", true)) {
        ...
        bridge_cfg.max_server_levels =
            static_cast<int>(Config::get_or<int64_t>("signals.level_bridge.max_server_levels", 50));
        signal_level_bridge_ =
            std::make_unique<SignalLevelBridge>(*signal_level_gateway_, *signal_bus_, bridge_cfg);
        schedule_signal_level_cleanup_();
    ...
    setup_affinity_handler_(strat_cfg.bounce, strat_cfg.breakout, strat_cfg.leaderlag,
                            strat_cfg.flushreversal);

    ws->connect("ws://127.0.0.1:" + std::to_string(*port) + "/");
    feed_->start();

    auto allow_pats = Config::get_or<std::vector<std::string>>("universe.pool.allow_patterns",
    ...
    pending_candidates_ = init_universe_pool_(gateway, conn_info.id, conn_info.exchange_volume_mult,
                                              allow_pats, deny_pats);
    ...
    pool_fallback_timer_.async_wait([this](const boost::system::error_code& ec) {
        ...
        LOG_INFO("[Universe] Screener notifications not received within 10s — "
                 "activating {} candidates as fallback pool",
                 pending_candidates_.size());
    ...
    proc_cfg.hot_path_cpu = static_cast<int>(Config::get_or<int64_t>("perf.hot_path_cpu", 2));
    proc_cfg.spsc_queue_capacity =
        static_cast<size_t>(Config::get_or<int64_t>("perf.spsc_queue_capacity", 65536));
    ...
    processor_->set_pipeline_components(signal_bus_.get(), strategy_engine_.get(),
                                        risk_manager_.get(), executor_.get());

    // Wire BeastWsClient raw message callback to processor queue
    ws->set_on_raw_message(
        [this](const char* data, size_t size, uint64_t recv_ns, TraceId trace_id) {
            RawWsEvent event;
            // Skip messages that don't fit — they are snapshots/large payloads already
            // handled by the MarketDataFeed JSON path.
            if (size > RawWsEvent::kMaxPayload)
                return;
    ...
    LOG_INFO("[PipelineProcessor] Initialized (CPU={}, queue={})", proc_cfg.hot_path_cpu,
             proc_cfg.spsc_queue_capacity);
}
