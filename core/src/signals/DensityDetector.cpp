        ...
        if (level.size > 0) {
            ...
            if (it != tracked_.end()) {
                ...
                if (age < cfg_.fake_threshold) {
                    // Disappeared too quickly — likely a fake / spoofed order.
                    Signal s {
                    ...
                    bus_.publish(s);
                } else if (it->second.emitted) {
                    // BUG-1 FIX: confirmed density disappeared — strategies that
                    // depend on it (BounceFromDensity invalidation, post-entry close)
                    // must be notified so they can exit in time.
                    Signal s {
                        .kind = SignalKind::DensityRemoved,
                        .timestamp = now,
                        .ticker = ticker_,
