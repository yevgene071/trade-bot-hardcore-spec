...
function drawPriceChart(ctx, w, h, history) {
  ...
  const pad = { top: 10, right: 45, bottom: 20, left: 10 },
  ...
  history
    ...
    .reverse()
    .forEach((p, i) =>
      ctx.lineTo(toX(mids.length - 1 - i), toY(p.mid * (1 - p.volatility_1min_bps / 10000)))
    );
  ctx.closePath();
  ctx.fill();
  ...
  history.forEach((p, i) => {
    ...
    if (buyH > 0) {
      ...
      ctx.fillRect(x, pad.top + ph - buyH, barW, buyH);
    }
    if (sellH > 0) {
      ctx.fillStyle = 'rgba(239,68,68,0.4)';
      ctx.fillRect(x, pad.top + ph - buyH - sellH, barW, sellH);
    }
  });
