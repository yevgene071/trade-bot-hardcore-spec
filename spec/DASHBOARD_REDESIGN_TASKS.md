...
```
**Критерий:** Возможность отката `git checkout dashboard-v1-backup -- src/control/DashboardServer.cpp`
...
**Файлы:** `src/strategy/IStrategy.hpp`  
...
**Файлы:** `src/features/ChartHistory.hpp` (новый)  
...
**Файлы:** `src/marketdata/OrderBook.hpp`, `src/marketdata/OrderBook.cpp`  
...
**Файлы:** `src/strategy/LeaderLag.hpp`, `src/strategy/LeaderLag.cpp`  
...
**Файлы:** `src/strategy/BreakoutEatThrough.hpp`, `src/strategy/BreakoutEatThrough.cpp`  
...
**Файлы:** `src/strategy/BounceFromDensity.hpp`, `src/strategy/BounceFromDensity.cpp`  
...
**Файлы:** `src/strategy/StrategyEngine.hpp`, `src/strategy/StrategyEngine.cpp`  
...
**Файлы:** `src/control/TickerController.hpp`, `src/control/TickerController.cpp`  
...
**Файлы:** `src/control/DashboardServer.cpp` (метод `serialize_state_locked_()`)  
...
**Файлы:** `CMakeLists.txt`, `src/control/dashboard.html` (новый), `src/control/DashboardServer.cpp`  
**Описание:**
1. Вырезать HTML/CSS/JS из `kDashboardHtml` в `src/control/dashboard.html`
2. В `CMakeLists.txt`: `add_custom_command` с `xxd -i dashboard.html > dashboard_html.h`
...
**ВАЖНО:** Все задачи Phase 4 — ПОСЛЕДОВАТЕЛЬНЫЕ. Они работают в одном файле `src/control/dashboard.html` и не могут быть распараллелены без merge-конфликтов. Порядок: DS-12 → DS-13 → DS-14..21 (последовательно) → DS-22 → DS-23.

### DS-12 — HTML: структура 8 вкладок
**Файлы:** `src/control/dashboard.html`  
...
**Файлы:** `src/control/dashboard.html` (секция `<style>`)  
...
**Файлы:** `src/control/dashboard.html` (секция `<script>`)  
...
**Файлы:** `src/control/dashboard.html` (секция `<script>`), `src/control/DashboardServer.cpp`  
**Описание:**
...
**Файлы:** `test/unit/dashboard_server_test.cpp`  
