...
bool DashboardServer::authorize(std::string_view header_value) const noexcept {
    ...
    // T4-SECURITY: [L-2] Avoid early exit on length mismatch to mitigate timing attacks.
...
void DashboardServer::update_state(const State& state) {
    ...
    LOG_INFO("Dashboard: broadcasting state update ({} bytes) to {} sessions. Equity={}", 
             payload.size(), snap.size(), current_state_.account.equity_usd);
...
void DashboardServer::start_ws_session(tcp::socket socket,
    ...
    auto ws_session = std::make_shared<Session>(std::move(socket), *this);
    {
        ...
        LOG_INFO("Dashboard: new session established. Total sessions: {}", sessions_.size());
    }
