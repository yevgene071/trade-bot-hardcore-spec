...
**Решение:** HTML вынесен в `core/src/control/dashboard.html`. На этапе сборки cmake вызывает `xxd -i` и генерирует `dashboard_html.h` с `const unsigned char dashboard_html[]`. В `core/src/control/DashboardServer.cpp` только `#include "dashboard_html.h"` и использование.
...
```
core/src/control/
  # DashboardServer.cpp — встроенный HTML-дашборд удалён при реорганизации.
  # Актуальный дашборд — standalone React-приложение в dashboard/
...
```bash
git checkout dashboard-v1-backup -- core/src/control/DashboardServer.cpp
cmake --build build
```
