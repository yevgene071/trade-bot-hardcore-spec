...
### 2.1. Transport Layer (`core/src/transport/`)
...
**Правило:** никакие типы из transport-слоя не утекают выше — всё конвертируется в доменные типы из `core/src/domain/`.

### 2.2. Domain (`core/src/domain/`)
...
### 2.3. Market Data (`core/src/marketdata/`)
...
- `ClusterSnapshot` (`core/src/marketdata/ClusterSnapshot.{hpp,cpp}`)
...
### 2.4. Feature Extractor (`core/src/features/`)
...
### 2.5. Signal Detectors (`core/src/signals/`)
...
### 2.6. Strategy Engine (`core/src/strategy/`)
...
### 2.7. Risk Manager (`core/src/risk/`)
...
### 2.8. Executor (`core/src/executor/`)
...
### 2.9. Logger & Journal (`core/src/logger/`)
...
### 2.11. Ticker Universe (`core/src/universe/`)
...
| Серверные уровни | WS `signal_level_subscribe` → `signal_level_triggered` | direct event-stream для `SignalLevelBridge`; `notification_subscribe` может содержать `SignalLevel`, но не является каноническим каналом trading logic |
...
### 2.12. External Feeds (`core/src/transport/external/`)
...
whitelist_tickers = ["BTC_USDT", "ETH_USDT", "SOL_USDT"]
...
После реорганизации (Этап 3 проекта):
...
│   ├── CMakeLists.txt
│   ├── CMakePresets.json
│   ├── conanfile.txt                   # libcurl, boost, spdlog, nlohmann_json, toml++, fmt, gtest
│   ├── .clang-format
│   ├── .clang-tidy
│   ├── src/
│   │   ├── main.cpp
│   │   ├── app/                        # BotApp оркестрация
│   │   │   ├── BotAppInit.cpp
│   │   │   └── BotAppRun.cpp
│   │   ├── transport/
│   │   │   ├── IHttpClient.hpp
...
│   │   │   ├── OrderGateway.{hpp,cpp}
│   │   │   ├── MarketDataFeed.{hpp,cpp}
│   │   │   ├── SignalLevelGateway.{hpp,cpp}
│   │   │   ├── FinresHandler.{hpp,cpp}
│   │   │   └── NotificationFeed.{hpp,cpp}
│   │   ├── domain/
│   │   │   └── Types.hpp
│   │   ├── marketdata/
│   │   │   ├── OrderBook.{hpp,cpp}
...
│   │   ├── risk/
│   │   │   ├── RiskManager.{hpp,cpp}
│   │   │   ├── NewsCalendar.{hpp,cpp}
│   │   │   └── AccountStatePersister.{hpp,cpp}
│   │   ├── executor/
│   │   │   ├── LiveExecutor.{hpp,cpp}
│   │   │   ├── PaperExecutor.{hpp,cpp}
│   │   │   ├── StartupRecovery.{hpp,cpp}
│   │   │   └── ActiveTrade.hpp
│   │   ├── control/
│   │   │   ├── DashboardServer.{hpp,cpp}
│   │   │   ├── KillSwitch.{hpp,cpp}
│   │   │   ├── ClockDriftMonitor.{hpp,cpp}
│   │   │   └── TickerController.hpp
│   │   ├── logger/
│   │   │   ├── Logger.{hpp,cpp}
...
│   │   │   └── Config.{hpp,cpp}
│   │   ├── trading/
│   │   │   └── OrderReconciliator.{hpp,cpp}
│   │   ├── metrics/
│   │   │   ├── MetricsRegistry.{hpp,cpp}
│   │   │   ├── MetricsExporter.{hpp,cpp}
│   │   │   └── AlertWebhook.{hpp,cpp}
│   │   └── perf/                        # Performance infrastructure
│   │       ├── CpuPinning.{hpp,cpp}
│   │       └── TimestampMonitor.{hpp,cpp}
│   ├── test/
│   │   ├── CMakeLists.txt
│   │   ├── unit/                       # ~60 unit-тестов
│   │   ├── integration/                # Интеграционные тесты
│   │   └── contract/                   # Контракт-тесты MetaScalp API
│   ├── bench/                          # Google Benchmark
│   │   ├── bench_perf.cpp
│   │   ├── bench_orderbook_apply.cpp
│   │   ├── bench_orderbook_depth.cpp
│   │   └── bench_tradestream_on_trade.cpp
│   └── tool/
│       └── labeler/                    # Инструмент разметки сигналов
│
├── dashboard/                          # React/Vite Web UI
├── docs/
│   └── spec/                           # Архитектура, стратегии, сигналы, API-контракт
├── data/                               # Runtime данные
│   ├── replay/dumps/                   # Сохранённые WS-сессии
...
├── metascalp-sdk/                      # SDK MetaScalp API
├── scripts/
│   ├── build.sh
│   └── test.sh
├── Makefile
...
1. **Фактический контракт API** — `METASCALP_API_CONTRACT.md`, сверенный с
   `../../metascalp-sdk/docs/MetaScalp-Api.md` (`metascalp-sdk` v1.0.7).
   Если поля/endpoint нет там, код не должен на него опираться без demo
   contract test.
2. **Единственное место парсинга JSON** — `transport/` слой. Выше — только доменные типы.
3. **OrderGateway отправляет только документированные request-поля.** Для
   `POST /api/connections/{id}/orders` это `Ticker`, `Side`, `Price`, `Size`,
   `Type`, `ReduceOnly`. `ClientId`, `ClientOrderId` и любые idempotency-key
   поля не отправляются, пока они не появятся в SDK/API contract.
4. **Имена полей в JSON — в одной таблице констант**, не разбросаны по коду:
   ```cpp
   ...
   ```
5. **Graceful degradation:** если в ответе появилось новое неизвестное поле — игнорируем; если исчезло известное — пишем WARNING, но не падаем.
6. **Ретраи** с экспоненциальным backoff только на сетевые ошибки и 5xx; на 4xx — сразу failure.
7. **Версия MetaScalp** в `/ping` логируется при старте; если major отличается от ожидаемого — большое жёлтое предупреждение в лог.
8. **Bootstrap стакана:** SDK v1.0.7 схема
   `orderbook_subscribe` с `FetchSnapshot=false` + отдельный REST
   `orderbook-snapshot` seed поддерживается через
   `[feed].fetch_snapshot_on_subscribe=false`. Safe default — `true`, то есть
   MetaScalp сам присылает WS snapshot. При `501 Not Implemented`
   exchange/ticker помечается как WS-snapshot-only.

---

## 9. Что НЕ делаем

- Никаких REST-poll для данных, которые есть по WS (orders/positions/balances) — с **единственными исключениями**: ambiguous-submit reconciliation после `POST /api/connections/{id}/orders` (см. § 2.8 + README OQ-1), startup-recovery (§ 2.8 T4-RECOVERY) и one-shot `orderbook-snapshot` seed/resync для локальной книги.
